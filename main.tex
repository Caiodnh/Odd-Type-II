...
Recall that for a superalgebra $S$, the superalgebra $S\sop$ is defined by $S\sop = S$ as superspaces, but with a different product. For every element $s\in S$, we denote it by $\bar s$ when regarded as an element of $S\sop$, and the product on $S\sop$ is defined by $\bar a \bar b = \sign{a}{b} \overline{ba}$ for all $a, b \in S$.
% \begin{notation}
...
\begin{thm}\label{thm:vphi-iff-vphi0-and-B}
    ...
    %
    \begin{equation}\label{eq:superadjunction}
        \forall r\in R,\,\forall u, v \in \U,  \quad B(ru,v) = \sign{r}{u} B(u,\vphi(r)v).
    \end{equation}
    %
...
As before, we suppose $\D$ is a graded division superalgebra, $\U$ is a nonzero right graded module of finite rank over $\D$ and $\vphi$ is a super-anti-automorphism on $R$.

Given a super-anti-automorphism $\vphi_0$ on $\D$ and a $\vphi_0$-sesquilinear form on $\U$,  

\begin{defi}
    Let $\vphi_0$ be a super-anti-automorphism on $\D$ and let $B$ be a $\vphi_0$-sesquilinear form on $\U$. We define 
\end{defi}
...
\begin{prop}\label{prop:barB-determines-vphi-inv}
    Let $\bar B (u,v) \coloneqq \sign{u}{v} \vphi_0\inv (B(v, u))$. 
    Then $\bar B$ is a $\vphi_0\inv$-sesquilinear form of the same degree and parity as $B$ and 
    %
    \begin{equation}\label{eq:barB-superadjunction}
        \forall r\in R,\,\forall u, v \in \U,  \quad \bar B(ru,v) = \sign{r}{u} \bar B(u,\vphi\inv (r)v).
    \end{equation}
    %
\end{prop}

\begin{proof}
    Since $B$ is $\FF$-bilinear, then $\bar B$ is $\FF$-bilinear. Also, since $\vphi_0$ preserves degree and parity, $\bar B$ is homogeneous of the same degree and parity as $B$. 
    ...
    \begin{align*}
       \intertext{ Condition \eqref{enum:linear-on-the-second}:}
        ...
        & = (-1)^{|u||v| + |u||d| + |d||B| + |d||v|} (-1)^{|d| (|B| + |v| + |u|) }  \vphi_0\inv \big(B(v, u)) d \\ &= \sign{u}{v} \vphi_0\inv \big(B(v, u)) d = \bar B(u, v) d .
    \intertext{Condition \eqref{enum:vphi0-linear-on-the-first}:}
        \bar B(ud, v) &= (-1)^{(|u| + |d|) |v|} \vphi_0\inv \big( B(v, ud) \big) \\ &= (-1)^{(|u| + |d|) |v|} \vphi_0\inv \big( B(v, u)d \big) \\ &= (-1)^{(|u| + |d|) |v|} (-1)^{|d| (|B| + |v| + |u|)} \vphi_0\inv (d) \vphi_0\inv\big( B(v, u) \big) \\ &= (-1)^{|u||v| + |d| |B| + |d||u|} \vphi_0\inv (d) \vphi_0\inv\big( B(v, u) \big) \\ &= (-1)^{(|B| + |u|) |d|} \vphi_0\inv (d) \bar B(u, v).
      \intertext{Finally, for Equation \eqref{eq:barB-superadjunction}, note that replacing $r$ for $\vphi(r)$ in Equation \eqref{eq:superadjunction} it becomes}
...
Recall that $\vphi_0$ and $B$ are not uniquely determined. 
Let $d\in \D$ and consider $\vphi_0' \coloneqq \operatorname{sInt}_d\circ\, \vphi_0$ and $B' \coloneqq d B$. 
If we define $\bar B' \coloneqq \sign{u}{v} \vphi_0\inv (B(v, u))$ and let $\bar B$ be as in Proposition \ref{prop:barB-determines-vphi-inv}, then we have that
\begin{align*}
    \bar B' (u,v) &= \sign{u}{v} (\vphi_0\inv \circ \operatorname{sInt}_{d\inv})  (d B(u, v) ) \\
    &= \sign{u}{v} \vphi_0\inv \big( (-1)^{|d| (|d| + |B| + |u| + |v|)}\,  d\inv d B (u, v) d \big) \\ &= \sign{u}{v} (-1)^{|d|}\, \vphi_0\inv (d) \vphi_0\inv(B(v, u)) =  (-1)^{|d|}\,\vphi_0\inv (d) \bar B (u, v).
\end{align*}


\begin{cor}
    Suppose $\vphi^2 = \id$ and that $\FF$ is algebraically closed. Then $\vphi_0^2 = \id$.
\end{cor}

\begin{proof}
    First note that, since $B$ is nonzero and $\vphi_0$ is a bijection, $\bar B$ is nonzero. 
    Since $\vphi\inv = \vphi\inv$, using Equation \eqref{eq:barB-superadjunction}, Theorem \ref{thm:vphi-iff-vphi0-and-B} and Remark \ref{rmk:we-only-need-B'-nonzero}, we conclude that there is $d\in \D$ such that $\vphi_0\inv = \operatorname{sInt}_d \circ \,\vphi_0$ and $\bar B = dB$.
    
    Since both parity and degree of $B$ and $\bar B$ coincide, hence $d\in \D\even_e$. 
    Using that $\FF$ is algebraically closed, $d\in \FF$. 
    In particular, $\operatorname{sInt}_d = \id$ and, hence, $\vphi_0^2 = \id$.
\end{proof}
