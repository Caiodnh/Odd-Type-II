...
\begin{ex}\label{ex:FZ2xFZ2sop-iso-FZ4}
    ...
    Note that $S$ is isomorphic to $\FF\ZZ_2$, but this time with the nontrivial $\ZZ_2$-grading.
...
Throughout this section, we will assume that $\FF$ is an algebraically closed field and that $\Char \FF \neq 2$.
...
\begin{prop}\label{prop:superpolarization}
    ...
    \begin{equation}\label{eq:superpolarization}
        \forall a,b\in T, \quad \beta(a,b) = (-1)^{p(a) p(b)} \eta(ab) \eta(a)\inv \eta(b)\inv.
    \end{equation}
    %
...
\begin{proof}
    ...
    \begin{alignat*}{2}
        ...
        \\
        \iff&& \beta(b, a) &= (-1)^{p(a) p(b)} \eta(ab) \eta(a)\inv \eta(b)\inv.
    \end{alignat*}
...
With this we translate our task to the level of abelian groups. 
Instead of considering the graded superalgebra with super-anti-automorphism $(\D, \vphi_0)$, we can focus, instead, on the data $(T, \beta, p, \eta)$, where $T$ is an abelian group, $\beta$ is an alternating bicharacter on $T$, $p\from T \to \ZZ_2$ is a group homomorphism and $\eta\from T \to \FF^\times$ satisfies Equation \eqref{eq:superpolarization}. 
Also, the condition of $\vphi_0$ being superinvolution clearly corresponds to $\eta(t)^2 =1$ for all $t\in T$.

\begin{cor}\label{cor:beta-nondeg-elem-2-grp}
    If $\beta$ is nondegenerate, then $T$ is an elementary $2$-group.
\end{cor}
...
Combining Corollaries \ref{cor:beta-nondeg-elem-2-grp} and \ref{cor:odd-cant-have-order-2} and translating back to $(\D, \vphi_0)$, we get:

\begin{cor}\label{cor:no-odd-M-vphi}
    If $\D \iso M(m,n)$ and $\vphi_0$ is a superinvolution, then $\D = \D\even$, \ie, either $m=0$ or $n=0$. \qed
\end{cor}

As we will see in Chapter ??, this implies that all Type I gradings on the Lie superalgebras in the series $B$, $C$, $D$ and $P$ are even.
...
\begin{remark}
    Corollary \ref{cor:odd-cant-have-order-2} also gives us another proof for the fact that the associative superalgebra $Q(n)$ does not admit a superinvolution (Corollary \ref{cor:Q-no-spuerinv-center}). 
    Recall that $Q(n) \iso \End_\D (\U)$ (prop ??), where $\D= \FF\ZZ_2$ with nontrivial canonical grading and $\U$ is a right $\D$-module of finite rank. 
    If there was a superinvolution on $Q(n)$, by Theorem \ref{thm:vphi-involution-iff-delta-pm-1} we would have a superinvolution on $\D$, a contradiction.
\end{remark}
...
will now show an example of a division grading on $M(1,1)$ with a super-anti-isomorphism preserving degrees. 
