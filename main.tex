...
\begin{defi}
    ...
    We say that $\mu$ is a \emph{(multiplicative) quadratic form} on $T$ if $\mu(t\inv) = \mu (t)$ for all $t\in T$ and the map $\xi\from T\times T \to \FF^\times$ such that
    \begin{equation}\label{eq:quadratic-form}
        \forall a,b\in T, \quad \xi (a,b) =  \mu(ab) \mu(a)\inv \mu(b)\inv.
    \end{equation}
    We say that the bicharacter $\xi$ is the \emph{polarization} of $\mu$.
\end{defi}

\begin{remark}
    In the presence of Equation \eqref{eq:quadratic-form}, the condition $\mu(t\inv) = \mu(t)$ is equivalent to $\mu(t^n) = \mu(t)^{n^2}$ for all $t\in T$, which justifies the nomenclature.
\end{remark}
...
\begin{proof}
    As a particular case of Proposition \ref{prop:superpolarization}, $\psi$ is an antiautomorphism if, and only, if 
    %
    \begin{equation}\label{eq:polarization}
        \forall a,b\in T, \quad \beta(a,b) = \mu(ab) \mu(a)\inv \mu(b)\inv.
    \end{equation}
...
It turns out that quadratic forms and their polarization are useful to construct abelian group extensions of $T$. 
The theory of group extensions go well beyond what we are going to use here. 
The reader who is interested can check ?? and ??. 
For our purposes, we will need the following concepts:
% We will now see Equation \eqref{eq:superpolarization} under a different perspective:

\begin{defi}
    Let $H$ be an abelian group and let $\sigma\from T\times T \to H$ be a (set theoretical) map.
    \begin{enumerate}[(i)]
        \item We say that $\sigma$ is a \emph{symmetric $2$-cocycle} if $\sigma(a,b) = \sigma(b,a)$ and $\sigma(a,b) \sigma(ab, c) = \sigma(a, bc) \sigma(b,c)$, for all $a,b,c \in T$;
        
        \item We say that $\sigma$ is a \emph{coboundary} if there is a map $f\from T \to H$ such that $\sigma(a,b) = f(ab)f(a)\inv f(b)\inv$ for all $a,b \in T$. In this case, we write $\sigma  = \mathrm{d} f$.
    \end{enumerate}
\end{defi}

It is easy to check that every symmetric bicharacter is a symmetric $2$-cocycle, and those are the only that we will need. 
It is also easy to check that every coboundary is a symmetric $2$-cocycle. 
Note that Equation \eqref{eq:superpolarization} states that if we take $H = \FF^\times$ and define $\sigma$ to be the bicharacter on $T$ given by $\sigma(a, b) \coloneqq (-1)^{p(a) p(b)} \beta(a,b)$ for all $a,b\in T$, then $\sigma = \mathrm{d} \eta$.

The condition of being a symmetric $2$-cocycle is precisely the one needed for the following definition:

\begin{defi}
    Let $H$ be an abelian group and let $\sigma\from T\times T \to H$ be a cocycle. 
    We define the abelian group $H \times_\sigma T$ to be the set $H \times T$ endowed with the operation given by
    \[
        (h_1, t_1)(h_2, t_2) = (h_1 h_2 \sigma(t_1, t_2), t_1 t_2),
    \]
    for all $h_1, h_2 \in H$ and $t_1, t_2 \in T$.
\end{defi}

\begin{prop}
    Let $H$ be an abelian group and let $\sigma_1, \sigma_2 \from T\times T \to H$ be symmetric $2$-cocycles. 
    If $\sigma_1 \sigma_2\inv$ is a coboundary, then $H \times_{\sigma_1} T \iso H \times_{\sigma_2} T$.
\end{prop}

\begin{proof}
    Let $f\from T\to H$ be a map such that $\sigma_1 \sigma_2\inv = \mathrm{df}$ and let $\alpha\from H \times_{\sigma_1} T \to H \times_{\sigma_2} T$ by $\alpha(h,t) = (h f(h), t)$.
\end{proof}
...
connect super-anti-automorphism and antiautomorphisms on $\D$ with different bicharacters. 

give us an interesting connection with the theory of group extensions (see ?? and ??). 
In both equations, the right hand side is a bicharacter
...
The support of the new grading will be the group $\widetilde T \coloneqq \{ \pm 1 \} \times_\sigma T$, where $\sigma\from T\times T \to \{\pm 1\}$ is given by $\sigma(a,b) = (-1)^{p(a)p(b)}$ for all $a,b \in T$. 
Also, we will denote by $\pi\from \widetilde T \to T$ the projection on the second entry.
...
\begin{proof}
    ...
    Let $T'$ be the support of such an 
