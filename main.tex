...
\begin{proof}
    ...
    \intertext{Condition \eqref{enum:vphi0-linear-on-the-first}:}
      ...
      \intertext{For Equation \eqref{eq:barB-superadjunction}, note that replacing $r$ for $\vphi(r)$ in Equation \eqref{eq:superadjunction}, it can be rewritten as}
...
\begin{thm}
    ...
    Let $\D$ be a graded division superalgebra such that $\D\even_e = \FF 1$ and let $\U$ be a nonzero right graded module of finite rank over $\D$. 
    Consider a super-anti-automorphism $\vphi_0$ on $\D$ and a nondegenerate $\vphi_0$-sesqui\-li\-near form $B$ on $\U$, and let $\vphi$ be the super-anti-automorphism on $R \coloneqq \End_\D (\U)$ determined by $(\vphi_0, B)$ as in Theorem \ref{thm:vphi-iff-vphi0-and-B}. 
...
\begin{proof}
    ...
    Therefore $\delta \in \{ \pm 1 \}$, so $B = \pm \overline{B}$, concluding the proof. 
\end{proof}

We will investigate one last question...

\begin{remark}\label{rmk:delta-equals-1}
    If $\vphi$ is a superinvolution, we can actually From Lemma...
\end{remark}
