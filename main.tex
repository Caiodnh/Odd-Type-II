...
\begin{lemma}\label{lemma:sesquilinear-form-iff-D-linear-map}
    The $\vphi_0$-sesquilinear forms $B:\U\times\U \to \D$ are in a one-to-one correspondence with the homogeneous $\D$-linear maps $\theta\from \U \to \U\Star$ via $B \mapsto \theta$ where $\theta(u) \coloneqq B(u, \cdot)$, for all $u \in \U$. Moreover, $B$ is nondegenerate if, and only if, $\theta$ is an isomorphism.
\end{lemma}

\begin{proof}
    Condition \eqref{enum:linear-on-the-second} of Definition \ref{def:sesquilinear-form} tell us that $\theta$ defined this way is, indeed, a map from $\U$ to $\U\Star$. 
    Also, it is easy to check that $\theta$ is homogeneous of same parity and degree of $B$.
    
    Recalling the left $\D$-action on $\U\Star$, condition \eqref{enum:vphi0-linear-on-the-first}, tell us   that, for all $u\in \U$ and $d\in \D$,
    \[
    ...
    To show that the correspondence is bijective, note that all the considerations above can be reversed, hence, given $\theta: \U \to \U\Star$, we can define a $\vphi_0$-sesquilinear form by $B(u, v) \coloneqq \theta(u)(v)$ .
    
    The moreover part follows from the fact that $\rad B = \ker \theta$, so the nondegeneracy of $B$ is equivalent to $\theta$ being injective. But $\U$ has finite rank over $\D$, which is the same rank of $\U\Star$ over $\D$, so $\theta$ is injective if, and only if, it is bijective.
\end{proof}
