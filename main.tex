...
\begin{ex}
    If we consider the associative superalgebra $Q(1)$ graded by the trivial group, it is a graded division superalgebra. 
    In this case $G^\# = \ZZ_2$ and, also, $T = \ZZ_2$. 
    Since $Q(1)$ is commutative, we have that $\beta$ is identically $1$.
    By Corollary \ref{cor:Q-no-spuerinv-center}, it does not admit a superinvolution, but the map $\eta\from \ZZ_2 \to \FF^\times$ given by $\eta (\bar 0) = 1$ and $\eta( \bar 1) = i$ gives a super-anti-automorphism on it. 
...
Corollaries \ref{cor:D-has-the-same-type} and \ref{cor:no-odd-M-vphi} show us that the presence of a superinvolution give a strong restriction on the gradings on $M(m,n)$: we do not have odd gradings. 
Nevertheless, we will show that we do have odd gradings on $M(m,n)\times M(m,n)\sop$ when $m=n$ (Example ??). 
For that we will use the concept of group extension.
% In our construction we are going to use the following concept:

\begin{defi}
    Let $H, K$ be (abelian) groups. 
    A \emph{(abelian) group extension of $H$ by $K$} is a (abelian) group $E$ together with group homomorphisms $\iota\from K \to E$ and $\pi\from E \to H$ such that 
    %
    ...
    $\iota$ is injective, $\pi$ is surjective and $\iota(K) = \ker \pi$ (and, hence, $H \iso \frac{E}{\iota(K)}$). 
    For shortness, we may refer to \eqref{eq:short-exact-sequence} as the extension itself. 
    An \emph{equivalence} between two extensions $0 \rightarrow K \xrightarrow{\iota} E \xrightarrow{\pi} H \rightarrow 0$ and $0 \rightarrow K \xrightarrow{\iota'} E' \xrightarrow{\pi'} H \rightarrow 0$ is an isomorphism $\alpha\from E \to E'$ such that the diagram
%     \begin{center}
% 		\begin{tikzcd}
...
%%%%%%%%%%
	\begin{center}
		\begin{tikzcd}
			&& |[alias=E]|E \arrow[to=E', "\alpha"] \arrow[to = H, "\pi"]&&\\
			0 \arrow[to=K]& |[alias=K]|K \arrow[to=E, "\iota"] \arrow[to=E', "\iota'"]&& |[alias=H]|H \arrow[to=0] & |[alias=0]|0\\
			&&|[alias=E']|E' \arrow[to=H, "\pi'"]&&
		\end{tikzcd}
	\end{center}
	commutes. 
	An equivalence from an extension to itself is called a \emph{self-equivalence}.
\end{defi}

Group extensions are usually studied in the context of group cohomology, which goes far beyond our scope.  
We will need only some simple concepts and elementary results involving then. 
The reader who is interested in the topic can check ?? and ??. 

Let us fix the abelian groups $H$ and $K$ for the remaining of the section.

\begin{defi}
    Let $H$ and $K$ be abelian groups and let $\sigma\from H\times H \to K$ be a map.
    \begin{enumerate}[(i)]
        \item We say that $\sigma$ is a \emph{symmetric $2$-cocycle} if $\sigma(a,b) = \sigma(b,a)$ and $\sigma(a,b) \sigma(ab, c) = \sigma(a, bc) \sigma(b,c)$, for all $a,b,c \in H$;
        
        \item We say that $\sigma$ is a \emph{$2$-coboundary} if there is a map $f\from H \to K$ such that $\sigma(a,b) = f(ab)f(a)\inv f(b)\inv$ for all $a,b \in H$. In this case, we write $\sigma  = \mathrm{d} f$.
    \end{enumerate}
\end{defi}

It is easy to check that every symmetric bicharacter is a symmetric $2$-cocycle, and those are the only we will need. 
It is also easy to check that every $2$-coboundary is a symmetric $2$-cocycle. 

The condition of being a symmetric $2$-cocycle is precisely the one needed for the following definition:
...
\begin{prop}
    Let $H$ be an abelian group and let $\sigma, \sigma' \from T\times T \to H$ be symmetric $2$-cocycles. 
    If $\sigma' \sigma\inv$ is a $2$-coboundary, then there is an isomorphism $\alpha\from H \times_{\sigma} T \to H \times_{\sigma'} T$ such that $\pi_{\sigma'} \alpha = \pi_\sigma$.
\end{prop}

\begin{proof}
    Take $f\from T\to H$ to be any map and consider $\alpha\from H \times_{\sigma} T \to H \times_{\sigma'} T$ defined by $\alpha(h,t) = ( f(t) h, t)$. 
    It is easy to check that $\alpha$ is a bijection. 
    
    Let us denote the product on $H \times_{\sigma} T$ by juxtaposition and the one on $H \times_{\sigma'} T$ by $*$.  
    If $h_1, h_2 \in H$ and $t_1, t_2 \in T$, then, on the one hand,
    \begin{align*}
        \alpha ((h_1, t_1)(h_2, t_2) ) &= \alpha (\sigma(t_1, t_2) h_1 h_2, t_1 t_2) = 
        ( f(t_1 t_2)\sigma(t_1, t_2) h_1 h_2, t_1 t_2)
    \end{align*}
    and, on the other hand,
    \begin{align*}
        \alpha ((h_1, t_1)) * \alpha ((h_2, t_2) ) &= ( f(t_1) h_1, t_1) * ( f(t_2) h_2, t_2) \\&= 
        (\sigma'(t_1, t_2) f(t_1) f(t_2) h_1 h_2, t_1 t_2).
    \end{align*}
    Hence $\alpha$ is an isomorphism if, and only if, $\mathrm{d}f = \sigma' \sigma\inv$. 
    The result follows.
\end{proof}

One should note that the concept of $2$-coboundary is related to super-anti-automorphisms. 
In fact, Equation \eqref{eq:superpolarization} states what $\mathrm{d} \eta$ is equal to.

Equation \eqref{eq:superpolarization} is linked to the concepts of quadratic form and $2$-cocycles. 
Let us explore this connection.

\begin{defi}
    Let $\mu\from T \to \FF^\times$ be a map. 
    We say that $\mu$ is a \emph{(multiplicative) quadratic form} on $T$ if $\mu(t\inv) = \mu (t)$ for all $t\in T$ and the map $T\times T \to \FF^\times$ defined by
    $(a,b) \mapsto \mu(ab) \mu(a)\inv \mu(b)\inv$
    is a bicharacter, which is referred to as the \emph{polarization} of $\mu$.
\end{defi}

\begin{remark}
    In the literature, the additive notation is usually used. 
    More generally, for a given commutative ring $\mathbb K$ and $\mathbb K$-modules $A$ and $B$, a \emph{quadratic map} $q\from A \to B$ is a map satisfying $q(kx) = k^2 x$, for all $k\in \mathbb K$ and $x \in A$, and that the map $A \times A \to B$ defined by $(x,y) \mapsto q(x+y) - q(x) - q(y)$ is bilinear. 
...
\begin{ex}\label{ex:quadratic-form-polarization-parity}
    Let $\mu\from T \to \FF^\times$ be given by $\mu(t) = 1$ for all $t\in T^+$ and $\mu(t) = i$ for all $t\in T^-$. 
    It is clear that $\mu(t\inv) = \mu(t)$. 
    Also, we have that $\mu(ab) \mu(a)\inv \mu(b)\inv = (-1)^{p(a)p(b)}$, which is a bicharacter.
\end{ex}

It is important to notice that even if $\eta$ determines a super-anti-isomorphism, it is not necessarily a quadratic form. 
...
\begin{prop}
    ...
    Denote by $\xi\from T\times T \to \FF^\times$ the map defined by $\xi(a, b) \coloneqq \mu(ab) \mu(a)\inv \mu(b)\inv$. 
    Then: 
    \begin{enumerate}[(i)]
        \item If $\xi = \beta$, then:
        
        $\mu$ is a quadratic form $\iff$ $\mu(t) \in \{\pm 1\}$ for all $t\in T$ $\iff$ $\psi$ is
        an involution on $\D$ (considered as an algebra);
        
        \item If $\xi = \tilde\beta$, then: 
        
        $\mu$ is a quadratic form $\iff$ $\mu(t)\in \{ \pm 1 \}$ for all $t \in T^+$ and $\mu(t)\in \{ \pm i \}$ for all $t \in T^-$ $\iff$ $\psi$ is a super-anti-automorphism such that $\psi^2$ is the parity automorphism on $\D$.
    \end{enumerate}
\end{prop}
...
\begin{proof}
    ...
    Analogously, from Equation \eqref{eq:superpolarization}, we have $\tilde\beta (t, t\inv) = \mu (tt\inv) \mu(t)\inv \mu(t\inv)\inv$, so $(-1)^{p(t)} = \mu(t)\mu(t\inv)$. 
    Therefore $\mu(t\inv) = \mu(t)$ if, and only if, $\mu(t)^2 = (-1)^{p(t)}$, proving (ii).
\end{proof}
...
\begin{prop}
    If $\beta$ takes values in $\{ \pm 1 \}$, then there are a quadratic forms on $T$ whose polarization are $\beta$ and $\tilde \beta$.
\end{prop}
...
\begin{proof}
    ...
    We then have $\bar\beta(a,b) = \bar\mu(ab) \bar\mu(a)\inv \bar\mu(b)\inv$ for all $a, b \in \overline T$.
    Using that $\barr \beta$ is alternating and taking $t = a = b$, we get $\bar \mu (t)^2 = \bar \mu(t^2) = \bar \mu (e) = 1$, for all $t\in \barr T$. 
    
    Now let $q\from T \to \barr T$ to be quotient projection and define $\mu\from T\to \FF^\times$ by $\mu (t) = \barr \mu (q(t))$ for all $t\in T$. 
    Then $\mu$ takes values in $\{ \pm 1 \}$ and $\beta(a, b) = \barr \beta (q(a), q(b)) = \barr \mu(q(a)q(b)) \barr \mu(q(a))\inv \barr \mu(q(b))\inv = \mu(ab) \mu(a)\inv \mu(b)\inv$, hence $\mu$ is a quadratic form with polarization $\beta$.
    
    To get a quadratic form with polarization $\tilde \beta$, simply multiply $\mu$ by the quadratic form of Example \ref{ex:quadratic-form-polarization-parity} (compare this with prop ??).
\end{proof}
...
We will denote by $\pi\from \widetilde T \to T$ the projection on the second entry.

\begin{lemma}\label{lemma:tildeT-finally}
    Let $T$ be a group, let $\beta$ be an alternating bicharacter on it taking values only in $\pmone$ and let $p\from T \to \ZZ_2$ be a group homomorphism. 
    The extensions corresponding to ?? and ?? are equivalent. 
    Further, we can embed the group $\pmone \times_\sigma T$ in $T \times \ZZ_4$.
\end{lemma}
...
\begin{proof}
    ...
    We will denote this grading by $\Delta$.
    
    Now suppose $\widetilde \Gamma$ is any grading on $(\mc E, \vphi)$ refining $\Gamma$, and consider $\widetilde T \coloneqq \supp \widetilde \Gamma$. 
    For each $\tilde t\in T$, there are unique $\lambda \in \pmone$ and $t\in T$ such that the component of $\tilde t$ with respect to $\widetilde \Gamma$
...
% \begin{remark}
%     The refinement above could have been construct using an automorphism on $\mc E$. 
%     Since $\beta$ takes values in $\{ \pm 1 \}$, instead of taking an involution on $\D$ we can take a super-anti-automorphism ...
% \end{remark}
