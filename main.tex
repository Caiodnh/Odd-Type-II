...
A degree preserving su\-per\--an\-ti\--auto\-mor\-phism $\vphi_0: \D \to \D$ can be seen as an isomorphism from $\D$ to $\D\sop$ and, hence, we we can use $\vphi_0$ and the left $\D\sop$-action on $\U\Star$ to define a left $\D$-action: $f\cdot d \coloneqq f\vphi_0(d)$, for all $f \in \U\Star$ and all $d\in \D$. 
Using this action, we have the following:

\begin{lemma}\label{lemma:sesquilinear-form-iff-D-linear-map}
    Let $\vphi_0\from \D \to \D$ be a degree preserving su\-per\--an\-ti\--auto\-mor\-phism. The $\vphi_0$-sesquilinear forms $B:\U\times\U \to \D$ are in a one-to-one correspondence with the homogeneous $\D$-linear maps $\theta\from \U \to \U\Star$ via $B \mapsto \theta$ where $\theta(u) \coloneqq B(u, \cdot)$, for all $u \in \U$ (or, reversely, via $\theta \mapsto B$ where $B(u,v) \coloneqq \theta(u)(v)$, for all $u, v\in \U$). Moreover, $B$ is nondegenerate if, and only if, $\theta$ is an isomorphism.
\end{lemma}
...
\begin{proof}
    ...
    To show that the correspondence is bijective, note that all the considerations above can be reversed: given $\theta: \U \to \U\Star$, we can define a $\vphi_0$-sesquilinear form by $B(u, v) \coloneqq \theta(u)(v)$.
...
By Lemma \ref{lemma:sesquilinear-form-iff-D-linear-map}, we have that $B$ is a nondegenerate $\vphi_0$-sesquilinear map. 
...
\begin{equation*}
    ...
    \end{split}
\end{equation*}
\ie, for every $r\in R$, we have that $\vphi(r)$ is its \emph{superadjoint} of $r$ with respect to $B$.

This discussion proves one direction of Theorem \ref{thm:vphi-iff-vphi0-and-B}, bellow.

\begin{defi}
    Let $d\in \D$. We define $\operatorname{sInt}_d\from \D \to \D$ to be the isomorphism given by $\operatorname{sInt}_d (c) \coloneqq \sign{d}{c} dcd\inv$, for all $c\in \D$.
\end{defi}
...
\begin{proof}
    Let $\vphi_0$ be a degree preserving super-anti-automorphism on $\D$ and $B: \U \times \U \to \D$ be a nondegenerate $\vphi_0$-sesquilinear form and consider $\theta$ as in 
    Lemma \ref{lemma:sesquilinear-form-iff-D-linear-map}. Then, from Equation \eqref{eq:superadjunction}, we have:
    %
    \begin{align*}
        \theta(ru)(v) &= \sign{r}{u} \theta(u)(\vphi(r)v)\\
        \theta (ru)(v) &= \sign{r}{u} \big(\theta (u)\circ \vphi(r)\big)(v)\\
        \theta (ru) &= \sign{r}{u} \theta(u) \circ \vphi(r) \addtocounter{equation}{1}\tag{\theequation}\label{eq:theta-is-R-superlinear}\\
        (\theta \circ r) (u) &= \sign{r}{u} (-1)^{(|\theta| + |u|)|r|} \big(\vphi(r)\big)\Star \big(\theta(u)\big)\\
    ...
    \end{equation}
    %
    Since $\U$ has finite rank over $\D$, the map $s \mapsto s\Star$ is invertible and, hence, $\vphi$ is uniquely determined and is, indeed, a super-anti-automorphism of $R$.
    
    For the ``moreover'' part, consider the left $R$-supermodule structure on $\U\Star$ by $r\cdot f = \sign{r}{f} r\circ f$. Then equation \eqref{eq:theta-is-R-superlinear}
    
    we to that Equation \eqref{eq:theta-is-R-superlinear} ca
\end{proof}
