...
\begin{lemma}\label{lemma:change-of-side-properties}
    ...
    Also, given another $R$-linear map $\theta: \V' \to \V''$, we have that $(\psi\theta)^\circ = \sign{\psi}{\theta} \theta^\circ\psi^\circ$. \qed
\end{lemma}
...
\begin{equation*}
    ...
    \end{split}
\end{equation*}
\ie, the super-anti-automorphism $\vphi$ corresponds to the \emph{superadjuntion} with respect to the form $B$. In particular, we can recover $\vphi$ from the pair $(\vphi_0, B)$.
