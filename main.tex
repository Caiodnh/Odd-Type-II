...
\begin{thm}\label{thm:vphi-iff-vphi0-and-B}
    ...
    If $\vphi$ is degree preserving super-anti-automorphism on $R \coloneqq \End_R(\U)$, then there is a pair $(\vphi_0, B)$, where $\vphi_0$ is a degree preserving super-anti-automorphism on $\D$ and $B\from \U \times \U \to \D$ is a nonzero $\vphi_0$-sesquilinear form, such that
    %
    \begin{equation}\label{eq:superadjunction}
    ...
    Moreover, another pair $(\vphi_0', B')$ determines the same super-anti-automorphism $\vphi$ if, and only if, there is a nonzero homogeneous element $d\in \D$ such that $\vphi_0 = \operatorname{sInt}_d \circ \vphi_0$ and $B'(u, v) = dB (u, v)$ for all $u, v \in \U$.
\end{thm}
...
\begin{proof}
    ...
    %
    \begin{alignat*}{2}
        \forall r\in R,\,\forall u, v \in \U, &&\;\; \theta (ru)(v)&= \sign{r}{u} \theta(u)(\vphi(r)v)
        \\&&&= \sign{r}{u} \big(\theta (u)\circ \vphi(r)\big)(v)\\
        \intertext{and, hence, equivalent to}
        \forall r\in R,\,\forall u \in \U,  && \theta (ru) &= \sign{r}{u} \theta(u) \circ \vphi(r). \addtocounter{equation}{1}\tag{\theequation}\label{eq:theta-is-almost-R-superlinear}\\
        \intertext{Recalling the definition of superadjoint operator, Equation \eqref{eq:theta-is-almost-R-superlinear} becomes}
        \forall r\in R,\,\forall u \in \U, && \;\;(\theta \circ r) (u) &= \sign{r}{u} (-1)^{(|\theta| + |u|)|r|} \big(\vphi(r)\big)\Star \big(\theta(u)\big) \\&&&=  \sign{r}{\theta} \big(\big(\vphi(r)\big)\Star \circ \theta \big) (u),
        \intertext{which is the same as}
        \forall r\in R, &&\theta \circ r &= \sign{r}{\theta}  \big(\vphi(r)\big)\Star \circ \theta.
        \intertext{In other words, we have}
        \forall r\in R, && \big(\vphi(r)\big)\Star &= \sign{r}{\theta}\, \theta \circ r \circ \theta\inv.\addtocounter{equation}{1}\tag{\theequation}\label{eq:vphi-r-Star-is-a-superconjugation}
        \end{alignat*}
    %
    % We have shown that
    % %
    ...
    if $B' = dB$, then it is clear from Equation \eqref{eq:superadjunction} that $B'$ determines the same $\vphi$ as $B$. 
    To prove the other direction, we consider, again, the left $R$-supermodule structure on $\U\Star$ give by Equation \eqref{eq:R-action-back-on-the-right} and let $\theta\from \U \to \U\Star$ be as above, \ie, $\theta(u) = B(u, \cdot)$. Similarly, let $\theta'\from \U \to \U\Star$ be defined by $\theta' \coloneqq B'(u, \cdot)$.
    ...
    %
    \begin{align*}
        \vphi_0' (c) B'(u, v) &= (-1)^{|c|( |B'| + |u| )}B'(uc, v) = (-1)^{|c|( |d| + |B| + |u| )} dB(uc, v) \\ &= \sign{c}{d} d\vphi_0(c) B(u,v) = \sign{c}{d} d\vphi_0(c)d\inv dB(u,v) \\ &= \sign{c}{d} d\vphi_0(c)d\inv B'(u,v),
    \end{align*}
    %
    hence $\vphi_0'(c) = \sign{c}{d} d\vphi_0(c)d\inv$, as desired.
\end{proof}
...
We will now describe $\vphi$ in terms of matrices with entries in $\D$. 
One could do that by following Equation \eqref{eq:vphi-r-Star-is-a-superconjugation}, but we are following a different path. 
Let $\{e_1, \ldots, e_k\}$ be a homogeneous $\D$-basis of $\U$ and let $\Phi = (\Phi_{ij}) \in M_k(\D)$ be the matrix such that $\Phi_{ij} = B(e_i, e_j)$. 
Given $r\in R$, suppose $r$ has matrix $X = (x_{ij})$ and $\vphi(r)$ has matrix $Y = (y_{ij})$. 
We can, then, rewrite Equation \ref{eq:superadjunction} as
%
\begin{align*}
    B(Xe_i, e_j) &= \sign{X}{e_i} B(e_i, Ye_j),
\intertext{which means}
    B\left(\sum_{\ell=1}^k e_\ell x_{\ell i}, e_j\right) &= \sign{X}{e_i} B\left(e_i, \sum_{\ell=1}^k e_\ell y_{\ell j}\right)\\
    \sum_{\ell=1}^k (-1)^{(|B| + |e_\ell|) |x_{\ell i}|} \vphi_0 (x_{\ell i}) B(e_\ell, e_j) &= \sum_{\ell=1}^k
\end{align*}
...
Our goal now is to specialize the results of the previous section to the case where $\vphi$ is a superinvolution. 
...
As before, we suppose $\D$ is a graded division superalgebra, $\U$ is a nonzero right graded module of finite rank over $\D$ and put $R = \End_\D (\U)$.
...
\begin{prop}\label{prop:barB-determines-vphi-inv}
    ...
    Further, if $B$ is nondegenerate and $\vphi$ is the super-anti-automorphism on $R$ determined by $(\vphi_0, B)$ via Theorem \ref{thm:vphi-iff-vphi0-and-B}, then 
    %
    \begin{equation}\label{eq:barB-superadjunction}
        \forall r\in R,\,\forall u, v \in \U,  \quad \overline {B}(ru,v) = \sign{r}{u} \overline {B}(u,\vphi\inv (r)v).
    \end{equation}
    %
\end{prop}

\begin{proof}
    Since $B$ is $\FF$-bilinear, then $\overline {B}$ is $\FF$-bilinear. Also, since $\vphi_0$ preserves degree and parity, $\overline {B}$ is homogeneous of the same degree and parity as $B$. 
    It remains to check the conditions of Definition \ref{def:sesquilinear-form} and Equation \eqref{eq:barB-superadjunction}.
    \vspace{2mm}
    \begin{align*}
    ...
    \intertext{Condition \eqref{enum:vphi0-linear-on-the-first}:}
      ...
      \intertext{Finally, for Equation \eqref{eq:barB-superadjunction}, note that replacing $r$ for $\vphi(r)$ in Equation \eqref{eq:superadjunction}, it can be rewritten as}
...
\begin{lemma}\label{lemma:bar-dB}
    Under the conditions of Definition \ref{def:barB}, let $d\in \D$ and consider $\vphi_0' \coloneqq \operatorname{sInt}_d\circ\, \vphi_0$ and $B' \coloneqq d B$. 
...
\begin{proof}
    ...
    Hence, for all $u, v \in \U$,
    %
    \begin{align*}
        \overline {B'} (u,v) &= \sign{u}{v} (\vphi_0\inv \circ \operatorname{sInt}_{d\inv})  (d B(u, v) ) \\
        &= \sign{u}{v} \vphi_0\inv \big( (-1)^{|d| (|d| + |B| + |u| + |v|)}\,  d\inv d B (u, v) d \big) \\ &= \sign{u}{v} (-1)^{|d|}\, \vphi_0\inv (d) \vphi_0\inv(B(v, u)) =  (-1)^{|d|}\,\vphi_0\inv (d) \overline {B} (u, v).
    \end{align*}
\end{proof}
...
\begin{thm}
    ...
    % Let $\D$ be a graded division superalgebra and let $\U$ be a nonzero right graded module of finite rank over $\D$. 
    Let $\vphi_0$ be a super-anti-automorphism  on $\D$, let $B$ be a nondegenerate $\vphi_0$-sesqui\-li\-near form on $\U$ and let $\vphi$ be a super-anti-automorphism  on $R$ determined by $(\vphi_0, B)$ by Theorem \ref{thm:vphi-iff-vphi0-and-B}. 
    Then $\vphi$ is a superinvolution if, and only if, $\vphi_0$ is a superinvolution and $B = \pm \overline B$. 
\end{thm}

\begin{proof}
    First of all, note that, since $B$ is nonzero and $\vphi_0$ is a bijection, $\overline {B}$ is nonzero.
    Using Equation \eqref{eq:barB-superadjunction}, Theorem \ref{thm:vphi-iff-vphi0-and-B} and Remark \ref{rmk:we-only-need-B'-nonzero}, we conclude that $\vphi = \vphi\inv$ if, and only if, there is $\delta \in \D$ such that $\vphi_0\inv = \operatorname{sInt}_\delta \circ \,\vphi_0$ and $\overline {B} = \delta B$.
    We will now prove that we always have $\delta = \pm 1$.
    % Assume that $\vphi^2 =\id_R$. 
     
    % Since $\vphi = \vphi\inv$,  there is $\delta \in \D$ such that $\vphi_0\inv = \operatorname{sInt}_\delta \circ \,\vphi_0$ and $\overline {B} = \delta B$.
    
    Both parity and degree of $B$ and $\overline {B}$ coincide, hence $\delta \in \D\even_e$. 
    Using that $\FF$ is algebraically closed, we have that $\D\even_e = \FF 1$, hence $\delta \in \FF 1$ and $\vphi_0\inv(\delta) = \delta$. 
    In particular, $\operatorname{sInt}_\delta = \id_\D$ and, therefore, $\vphi_0\inv = \vphi_0$. 
    Also, using Lemma \ref{lemma:bar-dB}, we have that
    \[
        B = \overline {\overline B} = \overline {\delta B} = (-1)^{|\delta|} \vphi_0\inv(\delta) \overline B= \delta \overline B = \delta^2 B.
    \]
    Since $B$ is nonzero, $\delta^2 = 1$ and, hence, $\delta = \pm 1$, as desired. 
\end{proof}
