...
Sometimes it will be useful to consider the superalgebra $\D$ as a regular algebra and look to antiautomorphisms on it. 
...
Note that, if $\beta$ takes values $\pm 1$, we cannot guarantee that $\D$ admits a superinvolution, as seen in the case $Q(1) \iso \FF\ZZ_2$, but it always admits an involution, as we will see in Proposition \ref{prop:D-has-involution}. %, in the usual sense. 
...
Nevertheless, we will show that we do have odd gradings on $M(m,n)\times M(m,n)\sop$ when $m=n$. 
To construct an example of that, we will use 
...
\begin{proof}
    First of all, note that every component of $\D \times \D\sop$ is $2$-dimensional. 
    Moreover, for every $t\in T$, the eigenspaces of the exchange superinvolution associated to $1$ and $-1$ are spanned by $(X_t, \overline X_t)$ and $(X_t, -\overline X_t)$, respectively. 
