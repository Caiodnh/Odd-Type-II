...
Consider the map $\tilde\vphi_0(d) := \sign{\vphi_1}{d}\vphi_1\inv d \vphi_1$ (where juxtaposition denotes composition of maps on the right).
Then
\begin{equation}\label{eq:sesquilinear-before-B}
    (ud)\vphi_1 = (u) (d\vphi_1) =  (u)(\vphi_1\vphi_1\inv d \vphi_1) = \sign{d}{\vphi_1}(u)\vphi_1 \,\tilde\vphi_0(d).
\end{equation}
It is straightforward to check that $\tilde\vphi_0: \D = \End_R(\mc U) \to \End_R(\mc U\Star) = \D\sop$ is an isomorphism of superalgebras.
...
Note that the isomorphism $\D\sop \to \D$ given by $d \mapsto d^\circ$ relates the right $\D\sop$-action on $\U\Star$ with the left $\D$-action: $f\cdot d = d^\circ \cdot f$.
...
We define $B: \U\times \U \to \D$ to be the $\FF$-bilinear map given by $B(u, v) = \vphi_1^\circ (u)(v)$. Considered as a $\FF$-linear map $B: \U\tensor \U \to \D$, it is homogeneous of the same parity and degree of $\vphi_1$.
...
% \]
% for all $u,v \in U$.
The map $B$ is \emph{$\vphi_0$-sesquilinear}, \ie, we have that
