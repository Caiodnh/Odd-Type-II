...
\begin{defi}
    Let $\V$ and $\V'$ be graded left $R$-supermodules and let $\psi: \V \to \V'$ be a $R$-linear map. We define $\psi^\circ: \V \to \V'$ to be the following map, written on the left:
    \[
...
We will now see that, considering ``scalars'' on $\D$, the map $B$ is \emph{$\vphi_0$-sesquilinear}, \ie, we have that
...
Since $\vphi_1^\circ (u) \in \U\Star$, we have that $\vphi_1^\circ(u)(vd) = \vphi_1^\circ(u)(v)d$, for all $u,v \in \U$ and $d\in \D$, which give us \eqref{eq:easy-one}.
