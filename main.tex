...
\begin{convention}\label{conv:pick-even-basis}
    If $\D$ is odd (\ie, $\D\odd \neq 0$), we will choose the $\D$-basis $\mc B$ to be one having only even elements. This is possible because, given any homogeneous $\D$-basis, we can multiply its odd elements by a nonzero homogeneous odd element in $\D$. 
\end{convention}
...
if the only $\vphi$-invariant  of $R$ are the trivial ones. 
...
After that, we consider $G$-gradings on these superalgebras and relate it with results from the previous sections.
...
We recall that superalgebras with super-anti-automorphism and their isomorphisms are special cases of Definitions {\tt ??} and {\tt ??}. 
...
The next definition and lemma give us a class of examples of superinvolution-simple superalgebras that are not simple only as superalgebras. 
In fact, Proposition \ref{prop:only-SxSsop-is-simple} tell us these are the only examples. 
...
\begin{proof}
    We will prove that the $\vphi$-invariant superideals of $(S \times S\sop, \vphi)$ are precisely the subsets of the form $I\oplus \vphi(I)$ where $I$ is a superideal of $S$. 
    
    Let $I$ be an superideal of $S$ and consider $J \coloneqq I \times I\sop$. 
    ...
    Now let $J$ be any $\vphi$-invariant superideal of $S\times S\sop$ and let $I \coloneqq (1,0)\, J \subseteq S$. 
    First, note that $I$ is a superideal of $S$.
    Since $J$ is an superideal, $I \subseteq J$, and since it is $\vphi$-invariant, $I + \vphi(I) = I \times I\sop \subseteq J$.
    If $(j_1, \bar j_2) \in J$, then, on one hand, $j_1 \in I$, and, on the other hand, $\vphi(j_1, \bar j_2) = (j_2, \bar j_1) \in J$, so $j_2 \in I$. 
...
\begin{ex}\label{ex:FxF-iso-FZ2}
    ...
    Then $S\times S\sop = \FF [\zeta]$ where $\zeta = (1, -1)$ and the exchange superinvolution is given by $\vphi(1) = 1$ and $\vphi(\zeta) = -\zeta$.
    Note that $\FF [\zeta] \iso \FF\ZZ_2$, where we are considering all elements of $\FF\ZZ_2$ to be even.
\end{ex}

\begin{ex}\label{ex:FZ2xFZ2sop-iso-FZ4}
    Consider $S = Q(1)$, so $S\even = \FF 1$ and $S\odd = \FF u$ where $u^2 =1$. 
    Note that $S$ is isomorphic to $\FF\ZZ_2$, but this time with a nontrivial $\ZZ_2$-grading.
    ...
    We claim that $R \coloneqq S\times S\sop$ is isomorphic to $F\ZZ_4$. 
    ...
    Also, the exchange superinvolution on $R$ is given by $\vphi(1) = 1$, $\vphi(\omega) = 1$, $\vphi(\omega^2) = -\omega^2$ and $\vphi(\omega^3) = -\omega^3$.
\end{ex}
...
\begin{proof}
    Suppose $R$ is not simple and let $I \subseteq R$ be a nontrivial superideal. 
    Note that $\vphi(I)$ is also a superideal, hence $I \cap \vphi(I)$ and $I + \vphi (I)$ are $\vphi$-invariant superideals. 
    Since $I \cap \vphi(I) \subseteq I \neq R$, we have that $I \cap \vphi(I) = 0$, so we can write $I + \vphi (I) = I \oplus \vphi (I)$. 
    Since $0 \neq I \subseteq I \oplus \vphi (I)$, we have that $R = I \oplus \vphi (I)$.
    Clearly, this implies $(R, \vphi)$ is isomorphic to $I \times I\sop$ with exchange superinvolution. 
...
Hence, Proposition \ref{prop:only-SxSsop-is-simple} tell us that if $(R, \vphi)$ is a superinvolution-simple superalgebra, then $R$ is isomorphic to either $M(m,n)$, $Q(n)$, $M(m,n) \times M(m,n)\sop$ or $Q(n) \times Q(n)\sop$.
However, it is well known that the associative superalgebra $Q(n)$ does not admit a superinvolution (see, for example, ?? and ?? or Corollary \ref{cor:Q-no-spuerinv-center}, bellow).
...
\begin{defi}
    Let $R$ be a associative (super)algebra.
...
\begin{proof}
    ...
    \intertext{hence,}
        \sum_{g\in G} c_g r = \sum_{g\in G} r c_g.
    \end{align*}
    %
    Since $s$ has a degree and $R = \bigoplus_{g\in G} R_g$, we must have $rc_g = c_g r$ for all $g \in G$. 
    By linearity, $r c_g = c_g r$ for all $r\in R$, hence $c_g \in Z(R)$.
\end{proof}

Applying this lemma to an associative superalgebra $R$, with $G^\#$ playing the role of $G$, we see that $Z(R)$ is a graded subsuperspace of $R$.
...
\begin{proof}
    The center of $Q(n)$ is isomorphic to $\FF1 \oplus \FF u$, where $u$ is an odd element of order $2$. 
    Let $\vphi$ is a super-anti-automorphism on $Q(n)$. 
...
\begin{proof}
    It is well known that $Z(M_{m+n}(\FF)) \iso \FF$, hence \eqref{item:F-id} follows. 
...
For the remaining of the section, we will consider, as before, $R \coloneq \End_\D(\U)$, where $\D$ is a graded division superalgebra and $\U$ is a nonzero graded right $\D$-module of finite rank. 
Also, let $\vphi$ be a super-anti-automorphism on $R$ and let $(\vphi_0, B)$ is be pair determining $\vphi$ as in Theorem \ref{thm:vphi-iff-vphi0-and-B} following Convention \ref{conv:pick-even-form}.
...
Indeed, for all $r\in R = \End_\D(\U)$ and all $u\in \U$, we have $r (r_c(u)) = r(uc) = (ru) c = r_c(r(u))$.

\begin{prop}\label{prop:R-and-D-have-the-same-center}
    The map $Z(\D) \to Z(R)$ given by $c \mapsto r_c$ is an isomorphism. 
    Moreover, $\vphi (r_c) = r_{\vphi_0(c)}$.
\end{prop}

\begin{proof}
    Given $r\in Z(R)$, we can define $c_r\in \End_R (\U) =\D$ by $uc_r = r(u)$ for all $u\in \U$. 
    Computations analogous of the ones above show that $c\in Z(\D)$, and it is clear that map $r\mapsto c_r$ is the inverse of the map $c \mapsto r_c$. 
    It is clear that these maps are superalgebra homomorphisms.
    ...
    Using that $B$ is nondegenerate, the results follows.
\end{proof}
...
\begin{proof}
    ...
    We will, first, extend this for superideals and, then, for $\vphi$-invariant superideals.
    
    If $I$ is a superideal, $M_k(I) = M_k(\FF) \tensor I$ is also a superideal since it is spanned by a set of $\ZZ_2$-homogeneous elements, namely, the elements of the form $E_{ij}\tensor d$ where $1 \leq i,j \leq k$ and $d \in I^\alpha$ for $\alpha \in \ZZ_2$. 
    Conversely, if $J = M_k(I)$ is a superideal, then we can write $I = \{ d\in  \D \mid E_{11}\tensor d \in J\}$. 
    For every $d\in I$, write $d = d_{\bar 0} + d_{\bar 1}$, where $d_\alpha \in \D^\alpha$. 
    Since the $\ZZ_2$-homogeneous components of $E_{11}\tensor d$ are $E_{11}\tensor d_{\bar 0}$ and $E_{11}\tensor d_{\bar 1}$, we have that $d_{\bar 0}, d_{\bar 1} \in I$.
    
    Now we are going to show that $M_k(I)$ is $\vphi$-invariant if, and only if, $I$ is $\vphi_0$-invariant. 
    Suppose $I$ is $\vphi_0$-invariant.
    Then if $X \in M_k(I)$, by the definition of $\vphi_0 (X\stransp)$, it is also in $M_k(I)$. 
    But then $\vphi(X) = \Phi\inv \vphi_0(X\stransp) \Phi \in M_k(I)$ since $M_k(I)$ is an ideal. 
    Conversely, suppose $M_k(I)$ is $\vphi$-invariant, let $d \in I$ and consider $X = E_{11} \tensor d$.
    Then $E_{11} \tensor \vphi_0(d) = \vphi_0(X\stransp) = \Phi\, \vphi(X)\, \Phi\inv \in M_k(I)$, concluding the proof.
\end{proof}
...
\begin{cor}
    ...
    %
     \begin{enumerate}[(i)]
        \item If $R\iso M(m,n)$, then there are $m', n'\in \ZZ^+$ such that $\D \iso M(m', n')$;
        \item If $R\iso M(m,n)\times M(m,n)\sop$ with exchange superinvolution, then there are $m', n'\in \ZZ^+$ such that $(\D, \vphi_0)$ is isomorphic to $M(m', n')\times M(m', n')\sop$ with exchange superinvolution;
        \item If $R\iso Q(n)\times Q(n)\sop$ with exchange superinvolution, then there is $n' \in \ZZ^+$ such that $(\D, \vphi_0)$ is isomorphic to $Q(n')\times Q(n')\sop$ with exchange superinvolution.\qed
    \end{enumerate}
\end{cor}
...
Throughout this section, we will assume that $\FF$ is an algebraically closed field and $Char \FF \neq 2$.
...
\begin{prop}\label{prop:superpolarization}
    ...
    \begin{equation}\label{eq:superpolarization}
        \forall a,b\in T, \quad \beta(a,b) = (-1)^{|a||b|} \eta(ab) \eta(a)\inv \eta(b)\inv.
    \end{equation}
    %
...
\begin{cor}
    Suppose $\vphi_0$ is a superinvolution. 
...
\begin{proof}
    ...
    Now let $t\in T^-$. 
    Every odd power of it is also an odd element, so $t$ cannot have an odd order. 
...
\begin{ex}\label{ex:FZZ_4-with-superinvoltion}
    ...
    The map $\theta'\circ \theta\inv: \D \to \D\sop$ corresponds to an super-anti-automorphism with the $\eta$ above.
\end{ex}
